...
export default function App() {
  ...
  const fetchForms = async () => {
    try {
      ...
      const settings = response.data.fieldSettings || {};
      const renames = response.data.columnRenames || {};
      setForms(active);
      setHiddenForms(hidden);
      setFieldSettings(settings);
      setColumnRenames(renames);

      // Fetch summary counts for all databases
      try {
        const summaryResp = await axios.get('/api/notification-summary');
        setDbNotificationCounts(summaryResp.data);
      } catch (e) {}
      
      // Auto-select "Become a Booster" if it exists and we're just starting
      if (!selectedForm && active.length > 0) {
        const mainForm = active.find((f: any) => f.title.toLowerCase().includes('become a booster'));
        if (mainForm) {
          setSelectedForm(mainForm.id);
        } else {
          setSelectedForm(active[0].id);
        }
      }
    } catch (err) {
  ...
  const renameForm = async (formId: string, customName: string) => {
    try {
      await axios.post('/api/rename-form', { formId, customName });
  ...
  const reorderForms = async (formId: string, direction: 'UP' | 'DOWN') => {
    ...
    try {
      await axios.post('/api/reorder-forms', { order: newForms.map(f => f.id) });
    } catch (err) {
  ...
  const toggleFieldVisibility = async (formId: string, status: string, field: string) => {
    ...
    try {
      await axios.post('/api/update-field-settings', { formId, status, hiddenFields: newHidden });
    } catch (err) {
  ...
  const updateLocalSchema = async (formId: string, schema: string[]) => {
    try {
      await axios.post('/api/update-local-schema', { formId, schema });
  ...
  const renameColumn = async (originalName: string, customName: string) => {
    try {
      await axios.post('/api/rename-column', { originalName, customName });
      setColumnRenames(prev => ({ ...prev, [originalName]: customName }));
  ...
  const updateBoosterField = async (id: string, field: string, value: string) => {
    try {
      await axios.post('/api/update-booster-field', { id, field, value });
  ...
  const fetchData = async (formIdTarget?: string) => {
    ...
    try {
      setRefreshing(true);
      const response = await axios.get('/api/booster-data', {
      ...
      // Local storage cache for speed
      localStorage.setItem(`cache_boosters_${idToFetch}`, JSON.stringify(data));
      localStorage.setItem(`cache_time_${idToFetch}`, new Date().toISOString());

      // Refresh summary counts when we load a form to keep it updated
      try {
        const summaryResp = await axios.get('/api/notification-summary');
        setDbNotificationCounts(summaryResp.data);
      } catch (e) {}
  ...
  const createLocalForm = async () => {
    ...
    try {
      const res = await axios.post('/api/create-local-form', { title: localFormTitle });
      setLocalFormTitle('');
      fetchForms();
      setSelectedForm(res.data.id);
    } catch (err) {
  ...
  const addLocalRow = async () => {
    ...
    try {
      await axios.post('/api/add-local-row', { formId: selectedForm, fields: newRowData });
      setNewRowData({});
      fetchData();
    } catch (err) {
  ...
  const addManualForm = async () => {
    if (!newFormId) return;
    try {
      await axios.post('/api/add-manual-form', { formId: newFormId });
      setNewFormId('');
      fetchForms();
    } catch (err) {
  ...
  const permanentDeleteForm = async (formId: string) => {
    ...
    try {
      await axios.post('/api/permanent-delete-form', { formId });
      fetchForms();
  ...
  const deleteForm = async (formId: string) => {
    ...
    try {
      await axios.post('/api/delete-form', { formId });
      fetchForms();
      if (selectedForm === formId) {
        const next = forms.find(f => f.id !== formId);
        setSelectedForm(next ? next.id : '');
  ...
  const restoreForm = async (formId: string) => {
    try {
      await axios.post('/api/restore-form', { formId });
      fetchForms();
    } catch (err) {
  ...
  const updateContactStart = async (id: string, contactType: 'TELEGRAM' | 'DISCORD' | null) => {
    try {
      await axios.post('/api/update-contact-start', { id, contactType });
